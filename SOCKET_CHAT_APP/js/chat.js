...
const token = `Admin ${localStorage.getItem("token")}`;
let globalProfile = {};
const headers = {
...
// clintIo.on('connect',()=>{
//     console.log("Connected to server with ID:", clintIo.id);
// })
...
clintIo.on('successMessage', (data) => {
    const { content, senderAvatar } = data
    const div = document.createElement('div');

    div.className = 'me text-end p-2';
    div.dir = 'rtl';
    const imagePath = senderAvatar ? `${baseURL}/upload/${senderAvatar}` : avatar;
    div.innerHTML = `
...
clintIo.on("newMessage", (data) => {
    ...
    if (from?.avatar) {
        imagePath = `${baseURL}/upload/${from.avatar}`
    }
    ...
    axios({
        ...
        if (chat) {
            if (chat.participants[0]._id.toString() == globalProfile._id.toString()) {
                meImage = chat.participants[0].avatar ? `${baseURL}/upload/${chat.participants[0].avatar}` : avatar
                friendImage = chat.participants[1].avatar ? `${baseURL}/upload/${chat.participants[1].avatar}` : avatar
            } else {
                meImage = chat.participants[1].avatar ? `${baseURL}/upload/${chat.participants[1].avatar}` : avatar
                friendImage = chat.participants[0].avatar ? `${baseURL}/upload/${chat.participants[0].avatar}` : avatar
            }
...
function getUserData() {
    axios({
        ...
        let imagePath = avatar;
        if (user.avatar) {
            imagePath = `${baseURL}/upload/${user.avatar}`
        }
...
// Show friends list
async function showUsersData(users = []) {
    ...
    for (let i = 0; i < users.length; i++) {
        let friend = await axios({
            method: 'get',
            url: `${baseURL}/api/user/${users[i]}/profile`,
            headers
        ...
        if (friend?.avatar) {
            imagePath = `${baseURL}/upload/${friend.avatar}`
        }
        cartonna += `
        <div onclick="displayChatUser('${friend._id}')" class="chatUser my-2">
        <img class="chatImage" src="${imagePath}" alt="" srcset="">
        <span class="ps-2">${friend.fullName}</span>
